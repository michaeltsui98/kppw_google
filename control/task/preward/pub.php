...
class Control_task_preward_pub extends Control_task_task{
	...
	function action_step2(){
		...
		if(Keke::formcheck($_POST['formhash'])){
			...
			->rule('task_cash', 'Keke_valid::large',array($_POST['task_cash'],$this->_conf['min_cash'],'任务赏金必须大于'.$this->_conf['min_cash']))
			->rule('need_work', 'Keke_valid::large',array($_POST['need_work'],1,'所需要稿件必须大于1'))
			;
			if($ck->check()===false){
