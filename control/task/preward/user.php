...
class Control_task_preward_user extends Control_user{
	 ...
	 * @var 二级菜单选中项,空值不做选择
	 */
	protected static $_left = 'preward';
	...
	function action_index(){
		...
		require Keke_tpl::template('control/task/preward/tpl/user/task_list');
	}
	...
	function pub_task($status,$cond=NULL){
		...
		$where .= "  and a.uid = ".self::$uid." and b.model_code = 'preward'";
	...
	function pub_task_status($task_info){
		$str = '';
		if($task_info['status_scode']=='wait'){
			$str .='<p>';
	...
	function action_edit(){
		...
		require Keke_tpl::template('control/task/preward/tpl/user/task_edit');
	}
	/**
	 * 保存任务信息
	 */
	function action_save(){
		...
		$this->request->redirect ( "task/preward_user/edit?task_id=$task_id" );
	}
	/**
	 * 删除任务
	 */
	...
	function action_seller(){
		...
		$base_uri = PHP_URL . "/task/preward_user/seller";
		
		$seller_work_uri = PHP_URL.'/task/preward_user/seller_work';
		...
		$sql = "SELECT \n".
				...
				"on b.uid  = e.uid\n";
				//"where a.uid = :uid and m.model_code = 'preward' \n";
		$sql= DB::query($sql)->tablepre('keke_')->compile(Database::instance());		 
		...
		$where .= "  and  a.uid = ".self::$uid." and m.model_code = 'preward'";
		...
		require Keke_tpl::template('control/task/preward/tpl/user/task_seller');
	}
	/**
	 * 我参与任务的稿件
	 */
	function action_seller_work(){
		...
		$base_uri = PHP_URL.'/task/preward_user/seller_work?task_id='.$_GET['task_id'];
		$sql="SELECT \n".
		...
		$where .= "  and a.uid = self::$uid and a.task_id = {$_GET['task_id']} and m.model_code = 'preward'  ";
		...
		require Keke_tpl::template('control/task/preward/tpl/user/seller_work');
	}
	...
	function action_wk_confirm(){
		...
		//$this->request->redirect ( "task/preward_user/seller?status=jf" );
	}
	/**
	 * 我参与搞件的操作
	...
	function join_task_op($v){
		if($v['task_status']=='6' && $v['sub_status']=='0'){
			$str = '<p><a onclick="return kdel(this,\'是否确认交付？\')" href="'.PHP_URL.'/task/preward_user/wk_confirm/'.$v['task_id'].'" >确认交付</a></p>';
