...
class Control_task_sreward_cron extends Sys_cron_task {
	...
	function jg_to_xg() {
		$sql = "update keke_witkey_task a \n".
				...
				"where b.model_code = 'sreward' and a.task_status =2 \n".
		...
		$sql = "select * \n".
				...
				"on a.model_id = m.model_id and m.model_code = 'sreward'\n".
		...
		if(sizeof($task_arr)>0){
			...
			$sql = "update \n".
					...
					"left join keke_witkey_model m \n".
					"on a.model_id = m.model_id and m.model_code = 'sreward'\n".
					"set a.task_status = 7,a.sub_status = 1\n".
					"where a.task_status = 2 \n".
					"and a.work_num <1 \n".
					"and from_unixtime(a.sub_time)<=now()";
	...
	function xg_to_gs() {
		...
		$sql = "update   (\n".
				...
				") t\n".
				"left join keke_witkey_model m \n".
				"on t.model_id = m.model_id and m.model_code = 'sreward'\n".
		...
		$sql = "update keke_witkey_task a \n".
				"left join keke_witkey_model b \n".
				"on a.model_id = b.model_id and b.model_code = 'sreward'\n".
				"left join keke_witkey_task_work w\n".
	...
	function gs_to_jf() {
		...
		$sql = DB::query($sql,Database::UPDATE)->tablepre('keke_')
		->param(':end_time', SYS_START_TIME+(self::$config['max_send']*3600*24))
		//->compile(Database::instance());
		//Log::instance()->add(Log::ALERT, $sql)->write();
		->execute();
	}
	function jf_to_end() {
