...
(function(c) {
	var b = {
		...
		cancel : function(d, f) {
			var e = arguments;
			var deleteFile = function(j, s, i, fid,fpath) {
				fid && c.getJSON(j.deleter + '&fid=' + fid+'&filepath='+fpath, function(json) {
					if (json.status == 1) {
						...
						if (e[0]) {
							if (e[0] == "*") {
								...
								for ( var m = 0; m < e.length; m++) {
									var q = c("#" + e[m]);
									var fid = q.attr("fid");
									var fpath = q.attr("fpath");
									deleteFile(j, i, q.index(), fid,fpath);
									i.cancelUpload(e[m]);
							...
							var fpath = $item.attr("fpath");
							deleteFile(j, i, $item.index(), fid,fpath);
			...
			if (c.inArray("onDialogClose", e.overrideEvents) < 0) {
				if (this.queueData.filesErrored > 0) {
					if(typeof art == 'function'){
						...
						art.dialog.alert(this.queueData.errorMsg);
		...
		onUploadComplete : function(f, e) {
			...
			if (c.inArray("onUploadComplete", g.overrideEvents) < 0) {
				if (g.removeCompleted) {
					...
					c("#" + f.id).attr("fpath", json.msg.url);
					switch (f.filestatus) {
